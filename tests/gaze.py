...
class GazeData:
    ...
    def __init__(self, folder='Data'):
        """
        Initialize the GazeData class.

        Args:
            folder (str): Folder containing gaze data files.
        """
        self.data_folder = folder
        recs = list(filter(lambda x: x != 'vecchi', os.listdir(folder)))
        self.recs = list(filter(lambda r: r.split('-')[1] == 'track_cal.txt', recs))
